...
function M.start_debugger(config, on_launch, on_exit, on_error, on_stderror)
  ...
  table.insert(args, "8123")
  table.insert(args, "127.0.0.1")
  ...
  stdout:read_start(function(err, chunk)
    ...
    local message = chunk:gsub("\n", "")
    local prefix = "Debug server listening at "
    if string.find(message, prefix) then
      local address = message:match(prefix .. "([a-z0-9:.]+)$")
      local splitter = address:match(".*():")
      port = address:sub(splitter + 1)
      host = address:sub(1, splitter - 1)
    else
      port = message
    end
    on_launch(port, host, proc)
  end)
